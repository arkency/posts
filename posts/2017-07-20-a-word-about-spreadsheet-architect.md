---
title: "A word about spreadsheet_architect"
created_at: 2017-07-20 17:59:07 +0200
kind: article
publish: false
author: anonymous
tags: [ 'foo', 'bar', 'baz' ]
...
Recently I heard about `spreadsheet_architect` gem and I wondered a few things after reading the readme which lead me to look into its code and wonder...
...
I was thinking about checking the code how it gets the list of records in implementing a compatible interface inside a different class. I was pretty sure the method was `#to_a` because why not? What else could it be? So I hoped for this kind of workaround:
...
So started looking into the code how it works internally to confirm my guesses.
...
Ok, so it turned out it is not just `to_a` but rather `where(nil).to_a`. I am not exactly sure would `where(nil)` be necessary. At this point I decided that workaround the API is probably not worth it and too hard.
...
Even though this is ActiveRecord::Relation the methods which are dynamically executed report that `self` is the `Post` class.
...
* as you can see the method is delegated to the class directly. So that's why `self` is `Post`.
* But why is only subset of records for generating the file, and not all, when the method is delegated directly to a class? Because `scoping` is used, which works like a global
...
In other words before `Post#to_xlsx` is called, `Post.current_scope` is set temporarily and as a result `Post.where(nil).to_a` called by the `SpreadsheetArchitect` is limited in scope and does not include all records.
