...
Two places where the same coupling remains, but just moved:
...
class Seat < ApplicationRecord
  ...
  # Invariant lives here. In one place. Th method always returns an event,
...
`Events.publish` does two things, both in the caller's transaction: it writes the event to a persistent log and synchronously calls subscribers. This is precisely the _RES_ mode — publish is a write to the _event store_ plus _dispatch_. First, the log: append-only, the source of truth, from which you replay:
...
```ruby
# publish: persisten fact + sync subscribers, atomically with the state change.
module Events
...
Here's all the persistence I need: `StoredEvent.append` runs in the same transactions as `seat.save!`, so the fact commits with a state change or not at all. Since the fact remains, any reaction can be recreated — handler retry, log replay. This is the invariant, and you have it without any additional machinery.
...
The correct answer ins't _outbox_ — it's the persistence of the fact. When `SeatReserved` is saved in the same transaction as the state change, no effect can be lost forever: since the fact remains, the reaction can always be recreated — retry the handler, replay from the log. This is the invariant in question, and you have it in bare _AR_ with a single `INSERT`. Outbox doesn't create this persistence — it only uses it, automating the delivery with an at-least-once guarantee. Therefore, this isn't a "scale-by" issue: you save the persistence of the fact from day one, and you harden its delivery method as and when necessary.
...
An invariant in reserve, a fact in `SeatReserved`, a state mutation in on — and this is an *event* that actually lands in the event store, with versioning and replay, not a fleeing notification named after a namespace. You attach handlers (mailer, webhook) as subscribers exactly as above.

And delivery? _RES_ provides exactly the distinction I outlined above. Publish within a domain transaction writes the _event_ to the _event store_ atomically with state changes and synchronously calls subscribers — you get a persistent, commit-couple event and immediate reactions, without any outbox (event store is equivalent to `StoredEvent`, and _publish_ is equivalent to `Events.publish`). You don't buy persistence in the gem; you get it from the _event store_ itself. `ruby_event_store-outbox` patches precisely the one gap I named above: transactional scheduling of asynchronous handlers so that their enqueue is atomic with the event. That's all — only maintained, resistant to multiple workers, and covered with mutation tests, nothing more than async dispatch.

A complete walkthrough of *RES* — aggregates, subscriptions, outbox, strangler on an existing monolith — is a topic for a separate, much longer text. Let me preface this point, as Brandon rightly dislikes rewrites; this path isn't rewriting. It's strangler — precisely the incremental movement it describes, callback by callback, flag by flat. The punchline here is enough: the structure he's approaching is available as a ready-made library, not an exotic one.
...
My point isn't "your pattern is too weak". It's: don't call it a _command_ and _event_ when it isn't. Because the different between a _procedure_ and a _command_, and between _notification_ and a _domain event_, isn't pedantry — it's the difference between "an invariant has an owner" and "an invariant is smeared, but nicely named".

I anticipate three counterargument:

"It's a dispute about names." Yes — and names are a contract that will reach 500 people. A bad mental mode scales worse than bad code, because code gets refactored, but beliefs don't.

"ES is overkill." Agreed, for most people. But a persistent record of a fact isn't _event sourcing_ — it's a single `INSERT` in the same transaction nas a state change, turning "effect can be lost" into "effect always recoverable". You don't skip this because you're not doing event sourcing; it's record path hygiene, not architecture.
...
Single-inress is correct. But an ingress that doesn't own the invariant isn't a fix — it's moving the same rule spread from a callback to a procedure. And calling that procedure a _command_ and the notification and _event_ installs a mental model for 500 engineers that doesn't fit the code they have in their hands.
