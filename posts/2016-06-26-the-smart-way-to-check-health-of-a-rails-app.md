...
## Implementation
We decided to implement `/health` in our app. Nonetheless, we agreed that it's a really good practice to do such checks in all of our apps and we released a tiny gem for that. Just to easily reuse this approach. The gem is named [wet-healt_endpoint](https://github.com/wetrb/wet-health_endpoint). Btw. we had to prefix _health_enpoint_ with something since all simple names are already taken in the _Rubygems_ world.
