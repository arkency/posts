...
You must not store as internal state change of aggregate and publish outside of
...
incomming messages and a [outbox pattern](https://microservices.io/patterns/data/transactional-outbox.html)
...
we still might have coupling on event's schema level.
...
In this solution each BC should have it's own private data store and specific
...
In modular-monolith application we could simplify this by usign only single instance
of Rails Event Store and separate domain events on streams level. This will require
more relying on an conventions and discipline of development team as there are no
...
to it's own streams and to "public" streams via the RES instance which is used to
communicate between bonded contexts.