...
- Rails expect that `.sprockets-manifest-totallyrandomdigest.json` will be present in `public/assets` when the app starts. Yep, digest included in manifest filename is totally random and Rails detects it based on some assumptions. So we use it to find our desired file: [Sprockets::Railtie.build_manifest(Rails.application).path](https://github.com/rails/sprockets-rails/blob/5badf679b206e4f218b9e3a42730d27779e572b2/lib/sprockets/railtie.rb#L213-L217). After that we're able to upload it under a known and predictable name: `manifest-$COMMIT_SHA.json`. We also produce `manifest-latest.json` as a fallback in case something went wrong and we haven't delivered manifest referencing released commit.
