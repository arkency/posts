...
I've just finished yesterday reading [Understanding the Four Rules of Simple Design](https://leanpub.com/4rulesofsimpledesign)
written by [Corey Haines](https://twitter.com/coreyhaines). I definitely
enjoyed reading it. The examples are small, understandable and good starting point
to show small refactions that follow. It's an inexpensive book, not very long, with
compressed knowledge and I can only recommend buying it. You can read it in a few hours, contemplate
for much longer.
...
This is not the right place this method. It should be in `...` I won't tell
you yet. Because quickly I realized that it could actually work...
...
This makes sense assuming infinite, 2-dimensional Map of square cells, which is the default for 
_Conway game of life_. But whenever you say _Game_ to me, first thing I think
about is _Civilization 5_ and it's hexagonal Map.
...
it's the location becoming to know what cell is on it, thus becoming a `Coordinate`:
...
* should `Location` know about its neighbors
* or should something else know what other locations are neighbors of given location.
That something would be probably `Map` for me.

What is the better dependency direction here?

Did we miss the concept of `Map` perhaps? Could we gain something by adding
it. Would it be more or less inention revelaing? I think these are good questions
to ask.
...
talks about _better desing_ concept.

## Month

A few months ago I wrote blogpost that shows [how to implement custom `YearMonth`
class in Ruby that would work with ruby Range](/2014/08/using-ruby-range-with-custom-classes/).

Basically `YearMonth` knows how to compute next one. It also works nicely with iterating
and comparison.
...
Days, Weeks, Months, and Years don't exist in a vacuum. They are parts of something
bigger. Passing time. That we follow with the usage of Calendar.

And I agree. There are even different kind of calendars in usage. I am not sure yet
if I have an intuition to design a good `Calendar` class with an API that would be 
nice to use. And how would I do that so all chunks of knowledge don't land in `Calendar` class
but in one proper place only?

## Employee

I was once writing an application for managing employee's holidays. In Europe you are allowed
to certain number of free days depending on how long have you been working in your life and
how long for that particular employer. And other factors as well.

The application that I was working was meant to be deployed separately per every company. So some of the queries
that I wrote were execute across entire sets in database. Without scoping per company because
the data was meant to be for that one company. That made the code easier in some places, I could
query things _globally_ when verifying correctness of some business rules.

One pivot later, the product was a SaaS intented to be deployed in one place but to support
multiple companies. The concept that I was reluctant to introduce has become immediatelly
necessary. Now everything had to be scoped per company of that employee currently using the
system. Multi-tenant application it was.

Employees don't live in a vacuum as well. There are part of a company hiring them. I was writing
